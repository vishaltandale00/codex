...
#[tokio::test]
async fn thread_merge_accepts_unrelated_threads() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
...
async fn thread_merge_rejects_duplicate_source_ids() -> Result<()> {
    ...
    let _request_id = mcp
        ...
        .await?;
    let error = match timeout(DEFAULT_READ_TIMEOUT, mcp.read_next_message()).await?? {
        JSONRPCMessage::Error(error) => error,
        message => panic!("expected JSONRPCMessage::Error, got {message:?}"),
    };
    assert!(error.error.message.contains("duplicate source thread id"));

    Ok(())
}

#[tokio::test]
async fn thread_merge_accepts_same_repo_root_threads() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    let repo_root = codex_home.path().join("repo");
    std::fs::create_dir_all(repo_root.join(".git"))?;
    let base_cwd = repo_root.join("workspace-a");
    let source_cwd = repo_root.join("workspace-b");
    std::fs::create_dir_all(&base_cwd)?;
    std::fs::create_dir_all(&source_cwd)?;

    let base_id = create_rollout_with_cwd(
        codex_home.path(),
        "2025-01-05T12-00-00",
        "2025-01-05T12:00:00Z",
        None,
    ...
    let source_id = create_rollout_with_cwd(
        codex_home.path(),
        "2025-01-05T12-01-00",
        "2025-01-05T12:01:00Z",
        None,
    ...
    let request_id = mcp
        .send_thread_merge_request(ThreadMergeParams {
            base_thread_id: base_id.clone(),
            merge_thread_ids: vec![source_id],
            ephemeral: true,
            ..Default::default()
        })
        .await?;
...
#[tokio::test]
async fn thread_merge_rejects_threads_from_different_repo_roots() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    let base_root = codex_home.path().join("repo-a");
    let source_root = codex_home.path().join("repo-b");
    std::fs::create_dir_all(base_root.join(".git"))?;
    std::fs::create_dir_all(source_root.join(".git"))?;
    let base_cwd = base_root.join("workspace");
    let source_cwd = source_root.join("workspace");
    std::fs::create_dir_all(&base_cwd)?;
    std::fs::create_dir_all(&source_cwd)?;
    ...
    assert!(
        error.error.message.contains("same workspace"),
        "expected workspace mismatch error, got {}",
        error.error.message
    );
...
#[tokio::test]
async fn thread_merge_rejects_request_cwd_outside_workspace() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    let base_root = codex_home.path().join("repo-a");
    let other_root = codex_home.path().join("repo-b");
    std::fs::create_dir_all(base_root.join(".git"))?;
    std::fs::create_dir_all(other_root.join(".git"))?;
    let base_cwd = base_root.join("workspace");
    let source_cwd = base_root.join("workspace-alt");
    let request_cwd = other_root.join("workspace");
    std::fs::create_dir_all(&base_cwd)?;
    std::fs::create_dir_all(&source_cwd)?;
    ...
    let _request_id = mcp
        .send_thread_merge_request(ThreadMergeParams {
            base_thread_id: base_id,
            merge_thread_ids: vec![source_id],
            cwd: Some(request_cwd.display().to_string()),
            ephemeral: true,
            ..Default::default()
        })
        .await?;
    ...
    assert!(
        error.error.message.contains("requested cwd"),
        "expected cwd workspace error, got {}",
        error.error.message
    );
...
#[tokio::test]
async fn thread_merge_accepts_exact_cwd_match_threads() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    let base_id = create_rollout_with_cwd(
        ...
        "2025-01-05T12:00:00Z",
        None,
        &[("base user", "base answer")],
        workspace.as_path(),
    )?;
    let child_id = create_rollout_with_cwd(
        codex_home.path(),
        "2025-01-05T12-01-00",
        "2025-01-05T12:01:00Z",
        None,
...
#[tokio::test]
async fn thread_merge_rejects_partial_repo_root_fallback() -> Result<()> {
    let server = app_test_support::create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    let base_id = create_rollout_with_turns(
        ...
        "2025-01-05T12:00:00Z",
        None,
        &[("base-1", "base-1 answer")],
    )?;
    let branch_id = create_rollout_with_cwd(
        codex_home.path(),
        "2025-01-05T12-01-00",
        "2025-01-05T12:01:00Z",
        None,
